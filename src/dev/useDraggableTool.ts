...
const storageKey = "park-acceptance-position";
...
export function useDraggableTool() {
  ...
  function move(next: Position) {
    ...
    setPosition(bounded);
    try { localStorage.setItem(storageKey, JSON.stringify(bounded)); } catch { /* Storage may be disabled. */ }
  }
  useEffect(() => {
    const rect = ref.current!.getBoundingClientRect();
    let initial = { x: rect.x, y: rect.y };
    try {
