...
export const reviewSchema = z.object({ id: z.string(), orderId: z.string(), direction: z.enum(["sent", "received"]), score: z.number().int().min(1).max(5), content: z.string(), createdAt: z.string(), anonymous: z.boolean().default(false), hidden: z.boolean().default(false), followup: z.object({ content: z.string(), createdAt: z.string(), hidden: z.boolean().default(false) }).optional(), reply: z.string().optional() });
...
export type AccountAction =
  ...
  | { type: "review"; orderId: string; score: number; content: string; anonymous: boolean }
